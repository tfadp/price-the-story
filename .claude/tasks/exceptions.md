...
- 2026-03-26 | backend/app/data/financial_datasets_client.py | Module-level httpx.AsyncClient singleton caused `RuntimeError: Event loop is closed` in tests. TestClient (anyio-backed) creates a fresh event loop per test; a singleton `AsyncClient` binds to the first test's loop and breaks for subsequent tests. Caught when tests 2, 3, and 13 failed with HTTP 500. Resolution: added `_FD_CLIENT_LOOP_ID` tracking — `_get_fd_client()` compares the current event loop's `id()` to the loop used when the singleton was created, and recreates the client if they differ. This preserves connection pooling in production (one loop per process) while staying safe in test environments.
