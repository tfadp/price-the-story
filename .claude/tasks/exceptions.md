...
- 2026-03-29 | backend/app/main.py + frontend/src/lib/api.ts | INTERNAL_API_KEY auth broke SSE entirely: browser EventSource cannot send headers, so any key set in the env caused all SSE requests to return 401. Additionally the POST helper in api.ts never sent the header. Caught in /review. | Backend now accepts key from x_api_key query param (for SSE) or x-api-key header (for POST). Frontend passes accordingly. No current behavior change — key is not set in .env.

- 2026-03-29 | backend/app/db/predictions.py + frontend/src/components/PredictionsTab.tsx | Scoring CAGR inconsistency: backend computed realized CAGR using elapsed wall-clock time (years_elapsed = days/365.25), modal previewed CAGR using holding_period_years. A prediction graded at 11 months would show different numbers in UI vs server. Caught in /review. | Both now use simple 1yr return (realized/entry - 1) matching the ledger's 1-year grading design.

- 2026-03-29 | backend/app/nodes/filings_rag.py | chromadb hard-imported at module level caused ModuleNotFoundError during pytest collection — before any tests or fallbacks could run. requirements.txt also missing edgartools, finnhub-python, chromadb, sentence-transformers. Caught in /review. | Moved chromadb import inside _get_collection() (lazy). Added all 4 Phase B packages to requirements.txt.

- 2026-03-29 | frontend/src/hooks/useAnalysis.ts | SSE stream not cancelled on reset: analyzeTickerStream returned void and discarded the EventSource handle. Starting a new analysis or resetting while a previous stream was active caused stale completion/error callbacks to overwrite newer UI state. Caught in /review. | analyzeTickerStream now returns a cancel function; useAnalysis stores it in a ref and calls it on reset() and at the start of each new run().
